...
case 'orderList':
	...
	if(isset($_POST['value']) && !empty($_POST['value'])) {
		$allowed_columns = ['trade_no','out_trade_no','name','money','domain','uid','status','type'];
		if(!in_array($_POST['column'], $allowed_columns)) exit('{"code":-1,"msg":"非法参数"}');
		if($_POST['column']=='name'){
			$sql.=" AND A.`{$_POST['column']}` like ?";
			$params[] = '%'.$_POST['value'].'%';
		}else{
			$sql.=" AND A.`{$_POST['column']}`=?";
			$params[] = $_POST['value'];
		}
	}
...
case 'riskList':
	...
	$params = [];
	if(isset($_POST['value']) && !empty($_POST['value'])) {
		$allowed_columns = ['uid','url','content','date'];
		if(!in_array($_POST['column'], $allowed_columns)) exit('{"code":-1,"msg":"非法参数"}');
...
case 'setStatus': //改变订单状态
	...
	if($status==5){
		...
		if($DB->exec("update pre_order set status=? where trade_no=?", [$status, $trade_no])!==false)
			exit('{"code":200}');
		else
...
case 'order': //订单详情
	$trade_no=trim($_GET['trade_no']);
	$row=$DB->getRow("select A.*,B.showname typename,C.name channelname from pre_order A,pre_type B,pre_channel C where trade_no=? and A.type=B.id and A.channel=C.id limit 1", [$trade_no]);
	if(!$row)
...
case 'operation': //批量操作订单
	...
	foreach($checkbox as $trade_no){
		...
		elseif($status==3){
			$row=$DB->getRow("select uid,getmoney,status from pre_order where trade_no=? limit 1", [$trade_no]);
		...
		else $DB->exec("update pre_order set status=? where trade_no=? limit 1", [$status, $trade_no]);
		$i++;
	}
...
case 'getmoney': //退款查询
	...
	$row=$DB->getRow("select * from pre_order where trade_no=? limit 1", [$trade_no]);
	if(!$row)
...
case 'refund': //退款操作
	...
	$row=$DB->getRow("select uid,money,getmoney,status from pre_order where trade_no=? limit 1", [$trade_no]);
	if(!$row)
		exit('{"code":-1,"msg":"当前订单不存在！"}');
	if($row['status']!=1)
		exit('{"code":-1,"msg":"只支持退款已支付状态的订单"}');
...
case 'apirefund': //API退款操作
	...
	if(!is_numeric($money) || !preg_match('/^[0-9.]+$/', $money))exit('{"code":-1,"msg":"金额输入错误"}');
	if(!(
    password_verify($paypwd, $conf['admin_paypwd'])
))
	exit('{"code":-1,"msg":"支付密码输入错误！"}');
	$row=$DB->getRow("select uid,money,getmoney,status,channel from pre_order where trade_no=? limit 1", [$trade_no]);
	if(!$row)
		exit('{"code":-1,"msg":"当前订单不存在！"}');
	if($row['status']!=1)
		exit('{"code":-1,"msg":"只支持退款已支付状态的订单"}');
