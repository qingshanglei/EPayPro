...
if($conf['login_alipay']==0 && $conf['cert_open']!=3 && !isset($_GET['app_auth_code']) && !isset($_GET['act']))sysmsg("未开启支付宝快捷登录");
...
}elseif(isset($_GET['app_auth_code'])){
	$channelid = intval(base64_decode($_GET['state']));
	$channel = \lib\Channel::get($channelid);
	if(!$channel)sysmsg('当前支付通道信息不存在');
	define("PAY_ROOT", PLUGIN_ROOT.'alipaysl/');
	require_once(PAY_ROOT."inc/AlipayAppAuthService.php");

	$oauth = new AlipayAppAuthService($config);
	$result = $oauth->getToken($_GET['app_auth_code']);
	if($result['app_auth_token']){
		...
		@header('Content-Type: text/html; charset=UTF-8');
		sysmsg('支付宝获取授权Token失败！['.$result['sub_code'].']'.$result['sub_msg']);
	}

}elseif(isset($_GET['act']) && $_GET['act']=='app_auth' && isset($_GET['channel'])){
	$channelid = intval($_GET['channel']);
	$channel = \lib\Channel::get($channelid);
	if(!$channel)sysmsg('当前支付通道信息不存在');
	define("PAY_ROOT", PLUGIN_ROOT.'alipaysl/');
	require_once(PAY_ROOT."inc/AlipayAppAuthService.php");
	$state = base64_encode($channelid);
	$oauth = new AlipayAppAuthService($config);
	$oauth->oauth($state);
