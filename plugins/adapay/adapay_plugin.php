...
class adapay_plugin
{
	...
	static public function alipay(){
		...
		$code_url = $result['expend']['qrcode_url'];
	...
	static public function bank(){
		...
		if($_SESSION[TRADE_NO.'_bank']){
			$result = $_SESSION[TRADE_NO.'_bank'];
		}else{
			try{
				$result = $app->submit('union_qr');
			}catch (Exception $e) {
				return ['type'=>'error','msg'=>'云闪付下单失败 '.$e->getMessage()];
			}
			$_SESSION[TRADE_NO.'_bank'] = $result;
		}
