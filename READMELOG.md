...
本次更新为项目首次全面安全加固迭代，涵盖SQL注入修复、XSS防护、CSRF防护、密码安全存储升级、硬编码密钥消除、Cookie安全加固等26+项安全漏洞修复，以及管理后台登录/退出循环Bug修复。经审计未发现恶意后门代码。在保留全部核心支付功能的前提下，消除了大量安全风险。
...
- **修复**：使用 bcrypt 哈希存储（`password_hash`/`password_verify`），移除旧版 `getMd5Pwd()` 函数，`verifyPwd()` 简化为纯 `password_verify()` 调用
- **影响文件**：
  - `includes/functions.php` — 删除 `getMd5Pwd()`，简化 `verifyPwd()` 为纯 `password_verify()`
...
- **问题**：登录错误5次后仅重置计数器允许再次尝试，攻击者可无限循环暴力破解密码
- **修复**：实现指数延迟锁定策略，错误次数越多锁定时间越长；锁定期间不验证密码，显示剩余等待时间
- **锁定规则**：
  | 错误次数 | 锁定时间 |
  |---------|---------|
  | 1-2次 | 无锁定，提示剩余尝试次数 |
...
- **原因**：`saveSetting()` 只写数据库不更新缓存，密码升级为bcrypt后，下次请求从缓存读取到旧明文密码，session计算结果不一致导致Cookie验证失败
...
- **原因**：安全修复后Cookie路径从默认的 `/admin688/` 改为 `/`，退出时仅删除 `/` 路径的Cookie，旧路径 `/admin688/` 的Cookie仍有效
...
- **问题**：登录错误5次后计数器直接重置为0，提示与密码错误相同，用户无法理解为何密码正确却登录失败
- **原因**：锁定检查在密码验证之后，且锁定后直接重置计数器允许无限重试，既不安全又误导用户
...
- **问题**：管理员密码修改表单缺少 `csrf_token` 隐藏字段，但服务端验证了csrf_token，导致密码修改永远失败
...
1. ✅ 管理员登录正常，密码验证使用 `password_verify`
2. ✅ 管理员退出登录后不再自动登录
3. ✅ 登录连续错误3次后锁定5分钟，锁定期间提示剩余等待时间
4. ✅ 管理后台所有配置保存正常（含cronkey等之前遗漏的键名）
5. ✅ 管理后台修改管理员密码和支付密码正常
6. ✅ 商户登录和密码修改正常
7. ✅ CSRF Token验证正常，非后台页面提交操作被拒绝
8. ✅ Cookie设置包含 `path=/`、`httponly=true`
9. ✅ SQL注入测试：动态列名白名单验证生效，非法列名被拒绝
10. ✅ 文件上传仅允许图片格式
