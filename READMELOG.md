...
本次更新新增vmqPro（v免签）支付插件，修复插件签名算法错误和回调通知异常，并修正Nginx伪静态规则配置问题。

## 🆕 新增功能

### vmqPro支付插件

- **说明**：新增v免签支付插件，支持微信支付、支付宝、QQ钱包三种支付方式
- **插件信息**：名称 `vmqPro`，作者 `青衫`
- **插件文件**：
  - `plugins/vmqPro/vmqPro_plugin.php`

## 🐛 问题修复

### v免签插件签名算法错误

- **问题**：原vmq插件创建订单签名缺少 `param` 字段，签名计算为 `md5(payId + type + price + key)`，v免签API要求 `md5(payId + param + type + price + key)`，导致签名永远不匹配，支付页面显示"about:blank#blocked"
- **修复**：补全 `param` 字段参与签名计算，即使 `param` 为空字符串也必须拼接
- **影响文件**：
  - `plugins/vmqPro/vmqPro_plugin.php`

### v免签插件多余参数mchId

- **问题**：原vmq插件发送了不属于v免签API的 `mchId` 参数
- **修复**：移除 `mchId` 参数，仅保留v免签API定义的字段
- **影响文件**：
  - `plugins/vmqPro/vmqPro_plugin.php`

### v免签插件回调验签缺少param字段

- **问题**：原vmq插件异步通知和同步回调验签均缺少 `param` 字段，导致支付成功后回调验签失败
- **修复**：notify/return 方法验签算法补全为 `md5(payId + param + type + price + reallyPrice + key)`
- **影响文件**：
  - `plugins/vmqPro/vmqPro_plugin.php`

### v免签插件price参数格式不一致

- **问题**：创建订单时 `price` 为PHP浮点数（如 `0.1`），v免签服务端回调时用 `number_format` 格式化为 `"0.10"`，格式不一致可能导致签名隐患
- **修复**：使用 `sprintf("%.2f", $order['realmoney'])` 统一为两位小数格式
- **影响文件**：
  - `plugins/vmqPro/vmqPro_plugin.php`

### v免签插件异步通知响应被输出缓冲污染

- **问题**：易支付 `pay.php` 在调用插件 `notify()` 前会 `include("common.php")`，可能向输出缓冲区写入额外内容，导致HTTP响应体为 `[额外输出]success` 而非纯 `success`，v免签服务端判定通知失败
- **修复**：在 `notify()` 方法开头添加 `ob_clean()` 清理输出缓冲区
- **影响文件**：
  - `plugins/vmqPro/vmqPro_plugin.php`

### Nginx伪静态规则错误导致回调404

- **问题**：宝塔面板配置的ThinkPHP通用规则 `rewrite ^(.*)$ /index.php?s=$1` 将 `/pay/notify/` 和 `/pay/return/` 请求路由到 `index.php` 而非 `pay.php`，导致所有支付插件的异步通知和同步回调返回404或首页HTML
- **修复**：替换为易支付官方伪静态规则，`/pay/` 路径独立rewrite到 `pay.php`，并添加 `/plugins/` 和 `/includes/` 目录访问禁止
- **影响文件**：
  - `/www/server/panel/vhost/rewrite/115.175.13.168_688.conf`

## 🎯 验证要点

1. ✅ 易支付后台配置vmqPro支付通道，填写接口地址、商户ID、通讯密钥后，测试支付功能正常打开支付页面（非"about:blank#blocked"）
2. ✅ 微信支付成功后，v免签监控端日志显示"通知回调状态：成功"（非"异步通知失败"）
3. ✅ 支付成功后，浏览器自动跳转回商户网站（同步回调正常）
4. ✅ 易支付后台订单状态正确更新为"已支付"
5. ✅ curl访问 `/pay/notify/订单号/` 返回纯文本 "success"（非HTML页面）
6. ✅ curl访问 `/plugins/` 返回403 Forbidden
...
1. ✅ 管理员登录成功后正常进入后台首页，不再跳回登录页
2. ✅ 管理员退出后显示"您已成功注销"，不再自动重新登录
3. ✅ 连续输错密码3次后显示锁定提示和剩余等待时间，锁定期间即使输入正确密码也无法登录
4. ✅ 修改支付密码时，输入旧密码后验证通过，新密码使用bcrypt存储
5. ✅ 修改管理员密码表单提交成功，不再提示"CSRF验证失败"
6. ✅ 新增支付通道、支付类型等操作提交成功，不再提示"CSRF验证失败"
7. ✅ 后台修改 `cronkey` 等之前遗漏的配置项，保存后生效
...
- **影响文件**：
  - `admin688/login.php` — admin\_token
  - `admin688/sso.php` — user\_token
...
1. ✅ 管理后台登录/退出/密码修改等核心操作正常，Cookie安全标志（HttpOnly/Secure/SameSite）正确设置
2. ✅ 管理后台所有AJAX接口携带CSRF Token，伪造Token的请求被拒绝
3. ✅ 商户注册/登录/密码修改使用bcrypt哈希存储，数据库中无明文密码
4. ✅ 管理后台配置修改接口无法覆盖 `admin_pwd`、`syskey` 等敏感配置
5. ✅ `authcode` 加解密使用数据库中的随机密钥，不同安装实例密钥不同
6. ✅ 文件上传仅允许图片格式，上传PHP文件被拒绝
7. ✅ 签名/密钥比较使用 `hash_equals()`，不存在时序攻击风险
